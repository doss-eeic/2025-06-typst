...
fn layout_inline_impl<'a>(
    ...
    let lines = match config.dir {
        Dir::LTR | Dir::RTL => linebreak(engine, &p, region.x - config.hanging_indent),
        Dir::TTB | Dir::BTT => linebreak(engine, &p, region.y - config.hanging_indent),
    };
